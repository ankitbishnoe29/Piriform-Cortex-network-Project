...
for pyr in pyrs:   
   ...
   for comp in (soma, dend1, dend2, dend3, dend4, dend5):
      ...
      noise.mean = 0
      noise.stdev = 0.092 #0.092 is control
      noise.seed(random.randint(0, 1000000))
      noise_list.append(noise)
...
for idx,Spike_times in enumerate(Spike_times_list):
    ...
    base_path = "/home/ankitk23/venv/Wopyr_pyr/"
    ...
    for i, fsi in enumerate(Fsis):
        neuron_spikes = pd.Series(fsi.spike_times)
        neuron_spikes.name = i+1  # Set the column name to the neuron index
        fsi_spike_data.append(neuron_spikes)
    ...
    for i, ff in enumerate(Ffs):
        neuron_spikes = pd.Series(ff.spike_times)
        neuron_spikes.name = i+1  # Set the column name to the neuron index
        ff_spike_data.append(neuron_spikes)
    ...
    pyrs_input = pd.DataFrame(tVecList_pyr)
    num_sets = math.ceil(len(pyrs_input) / number_of_sources_pyr)
    result_data = {}
    for i in range(num_sets):
        chunk = pyrs_input.iloc[i*23:(i+1)*23]
        combined_data = chunk.values.flatten()
        combined_data.sort()
        column_name = str(i + 1)
