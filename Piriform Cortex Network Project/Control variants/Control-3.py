...
for pyr in pyrs:   
   ...
   for comp in (soma, dend1, dend2, dend3, dend4, dend5):
      ...
      noise.mean = 0
      noise.stdev = 0.088 #0.092 is control
      noise.seed(random.randint(0, 1000000))
      noise_list.append(noise)
...
for idx,Spike_times in enumerate(Spike_times_list):
    ...
    base_path = "/home/ankitk23/venv/Control_all/Control-3"
    ...
    # Prepare voltage-time data
    voltage_data = {'Time': np.array(t)}
    selected_pyrs = random.sample(pyrs,1)
    # Collect voltage data for each pyramidal cell
    for i, pyr in enumerate(selected_pyrs):
        voltage_data[f'Voltage_{i+1}'] = np.array(pyr.soma_v)
        voltage_df = pd.DataFrame(voltage_data)
        vt_file_name = f'pyrvt{rank+1}.csv'
        vt_file_path = os.path.join(common_saving_path, vt_file_name)
        voltage_df.to_csv(vt_file_path, index=False)
